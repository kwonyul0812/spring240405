...
//    private Mapper01 mapper = new Mapper01();
    ...
    // todo : 직원 조회 및 수정
    @GetMapping("sub2")
    public void sub2(Integer id, Model model) throws Exception {
        if (id != null) {
            String sql = """
                    SELECT *
                    FROM Employees
    ...
    public String update2(MyBean258Employee employee, RedirectAttributes rttr) throws Exception {
        String sql = """
                UPDATE Employees
                SET LastName = ?,
                    FirstName = ?,
        ...
        try (conn; pstmt) {
            ...
            if (rowCount > 0) {
                ...
                rttr.addFlashAttribute("message", "수정 되지 않았습니다");
            }
        }
